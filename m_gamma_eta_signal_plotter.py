...
path = "samples/trees_v1.0/signal/"
innertree = "diphotonAnalyzer/fTree2"
outertree = "diphotonAnalyzer/fTree"
...
outputfile = ROOT.TFile("m_gamma_eta_signal_histograms.root", "recreate")
...
# Make TChain for outer loop
outerchain = ROOT.TChain(outertree)
rootfiles = []
for root, dirnames, filenames in os.walk(path):
  for filename in fnmatch.filter(filenames, '*.root'):
    rootfiles.append(os.path.join(root, filename))
for rootfile in rootfiles:
  outerchain.Add(rootfile)
...
eventInfo = ROOT.eventInfo_t()
outerchain.SetBranchAddress("Event", ROOT.AddressOf(eventInfo, "run") )
ROOT.gROOT.ProcessLine(
...
recoPhotonInfo = ROOT.recoPhotonInfo_t()
outerchain.SetBranchAddress("Photon1", ROOT.AddressOf(recoPhotonInfo, "pt") )

# Make TChain for inner loop
innerchain = ROOT.TChain(innertree)
rootfiles = []
for root, dirnames, filenames in os.walk(path):
  for filename in fnmatch.filter(filenames, '*.root'):
    rootfiles.append(os.path.join(root, filename))
for rootfile in rootfiles:
  innerchain.Add(rootfile)
ievent = array('i', [-1])
innerchain.SetBranchAddress("eventNum", ievent)
irun = array('i', [-1])
innerchain.SetBranchAddress("runNum", irun)
ilumi = array('i', [-1])
innerchain.SetBranchAddress("lumiNum", ilumi)
nPass = array('i', [-1])
innerchain.SetBranchAddress("nPass", nPass)
ROOT.gROOT.ProcessLine(
...
recoTwoProngInfo = ROOT.recoTwoProngInfo_t()
innerchain.SetBranchAddress("Eta1", ROOT.AddressOf(recoTwoProngInfo, "CHpos_pt") )

# make dictionary of event numbers
print "Making dictionary..."
EventNumbers = {}
for i in xrange(outerchain.GetEntries()):
  outerchain.GetEntry(i)
  EventNumbers[eventInfo.evnum] = i
print "Done."

total = innerchain.GetEntries()
for i in xrange(innerchain.GetEntries()):
  if i % 100000 == 0:
    percentDone = float(i) / float(total) * 100.0
    print 'Processing outer TTree {0:10.0f}/{1:10.0f} : {2:5.2f} %'.format(i, total, percentDone )
  innerchain.GetEntry(i)

  eventnum = ievent[0]
  run = irun[0]
  lumi = ilumi[0]

  EtaCand = ROOT.TLorentzVector()
  hasEta = False
